...
const ArtistLocation = () => {
  ...
  const {
    ...
    setValue,
    getValues,
    handleSubmit, // Add handleSubmit from react-hook-form
    formState: { errors },
  } = useForm<IFormInput>();
  ...
  // Renamed to avoid conflict with react-hook-form handleSubmit
  const onSubmitForm = async (data: IFormInput) => {
    try {
      ...
      router.push("/onboarding/artist/experience-rates");
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-4xl bg-surface rounded-xl shadow-sm p-6 ">
        {/* Added form handleSubmit */}
        <form
          className="space-y-4 w-full p-4 max-h-[calc(100vh-20rem)] overflow-y-auto"
          onSubmit={handleSubmit(onSubmitForm)} // Fixed form submission
        >
      ...
      <div className="w-full max-w-4xl bg-transparent">
        <div className="flex space-x-2">
          <CButton
            ...
            onClick={() => router.push("/onboarding/artist/genres")}
            className="w-full"
          >
            Back
          </CButton>
          {/* Fixed the onSubmit call */}
          <CButton
            loading={btnLoading}
            variant={ButtonType.Primary}
            onClick={handleSubmit(onSubmitForm)} // Fixed this line
            className="w-full"
          >
            Continue
          </CButton>
