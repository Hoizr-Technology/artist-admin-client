...
import { allGenres } from "@/utils/enums/genres.enum"; // Array of {value, label, icon}
...
const ArtistGenres = () => {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredGenres.length / ITEMS_PER_PAGE)
  );
  ...
  return (
    <motion.div className="w-full max-w-5xl mx-auto mb-auto min-h-[calc(100vh-111px)] sm:px-6 flex flex-col justify-between bg-transparent items-center text-center relative py-6">
      ...
      <p className="max-w-xl mx-auto text-white/60 mb-5 text-sm">
        Choose genres that best reflect your artist identity. This helps us
        recommend you for the right gigs.
      </p>
      ...
      {/* SCROLLABLE Genre Grid & Chips */}
      <div
        className="w-full flex-1 overflow-y-auto mb-4"
        style={{
          maxHeight: "calc(100vh - 370px)", // Responsive for most mobile screens
          minHeight: 0,
        }}
      >
        {/* Selected Genres */}
      ...
      <div className="w-full flex flex-col sm:flex-row justify-between items-center sm:space-x-6 space-y-2 sm:space-y-0 mt-6">
        <CButton
