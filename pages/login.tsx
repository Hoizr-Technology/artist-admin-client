...
const Login: FC = () => {
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    try {
      setBtnLoading(true);
      const response = await sdk.artistLogin({ email: sanitizeText(email) });

      if (response.artistLogin !== null) {
        setShowModal(true);
        if (response.artistLogin !== "") {
          setToastData({
  ...
  const onSubmitOtp: SubmitHandler<IFormInput> = async () => {
    try {
      ...
      const response = await sdk.artistLoginVerification({
        input: {
          otpId: otpId,
      ...
      if (response && response.artistLoginVerification) {
        setBtnLoading(false);
